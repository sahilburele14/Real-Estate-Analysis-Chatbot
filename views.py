...
The area shows a price appreciation of {growth_rate:.1f}% over the analyzed period, with average prices moving from ₹{avg_price_start:,.0f} to ₹{avg_price_end:,.0f} per square foot.

Demand trends are {demand_trend}, with current demand index at {data['demand'].iloc[-1]:.0f}. The locality has maintained an inventory of approximately {data['inventory'].iloc[-1]:.0f} units.

This area represents {"strong growth potential" if growth_rate > 10 else "stable investment opportunity"} with {"high" if data['demand'].iloc[-1] > 80 else "moderate"} market demand."""
